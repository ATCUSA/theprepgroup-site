<script lang="ts">
  ...
  function getModalTitle(): string {
    return actionType === 'approve' 
      ? `Approve Request from ${selectedRequest?.name}`
      : `Reject Request from ${selectedRequest?.name}`;
  }
</script>
        ...
        <table class="min-w-full divide-y divide-gray-700">
          ...
          <tbody class="bg-gray-800 divide-y divide-gray-700">
            {#each data.pendingRequests as request}
              <tr>
                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-300">{request.name}</td>
                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-300">{request.email}</td>
                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-300">{request.zip_code}</td>
                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-300">{formatDate(request.created_at)}</td>
      ...
      <form method="POST" action={getFormAction()} use:enhance={() => {
        ...
        <div class="px-6 py-4">
          ...
          <div class="mb-4">
            ...
            <p class="text-gray-300 mb-2">
              <strong>Zip Code:</strong> {selectedRequest.zip_code}
            </p>
            <p class="text-gray-300 mb-2">
